  ...
  <el-col :span="24" class="header">
    ...
    <el-col :span="4" class="userinfo">
      <el-dropdown trigger="hover">
        <span class="el-dropdown-link userinfo-inner"><img :src="this.sysUserAvatar" /> 鱼鱼</span>
...
export default {
  ...
  methods: {
    //退出登录
    logout: function () {
      this.$ajax.get('/api/workorder/calllog.php/tracklist', {
        limit: 1111,
        phone: 2222
      }).then((res) => {
        // 这里处理拿到的数据
        if (res.status === 200) {
          ...
          this.$message.error('来电轨迹信息获取失败')
        }
      }).catch((e) => {
        // 有异常这里会输出
        this.$message.error('来电轨迹接口请求失败')
        console.log(e)
      })
    },
    //折叠导航栏
